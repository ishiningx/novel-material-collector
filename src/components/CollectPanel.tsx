            ...
            {loading ? (
              ...
              <>
                ...
                <button
                  ...
                  className="px-3 py-1.5 rounded-full text-sm transition-all border border-dashed border-gray-300 dark:border-dark-100 text-gray-400 hover:text-primary hover:border-primary/50 flex items-center gap-1"
                >
                  <Plus size={12} />
                  添加新分类
                </button>
          ...
          {showAddCategory && (
            <div className="mt-2 flex items-center gap-2">
              <input
                ...
                placeholder="输入分类名称"
                autoFocus
                className="flex-1 text-sm px-3 py-1.5 rounded-full ring-1 ring-inset ring-gray-200 dark:ring-dark-100 bg-surface dark:bg-dark text-gray-900 dark:text-gray-100 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary/30 focus:ring-inset"
              />
              <button
                onClick={handleAddCategory}
                disabled={!newCategoryName.trim() || addingCategory}
                className="px-3 py-1.5 text-sm rounded-full font-medium transition-colors bg-primary text-[var(--skin-btn-text)] hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {addingCategory ? '...' : '添加'}
              </button>
              <button
